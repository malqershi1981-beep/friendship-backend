...
// Configure nodemailer transporter: prefer real SMTP, otherwise create Ethereal test account
...
console.log("SMTP transporter created");
      ...
      console.log("Using Ethereal test account (preview emails):", testAccount.user);
    }
  } catch (err) {
    console.warn("Failed to configure SMTP transporter:", err);
  }
})();
...
try {
  ...
  console.log("Could not parse SMTP send info", e);
      ...
      try {
        ...
        if (preview) console.log(`Order email preview URL: ${preview}`);
      } catch (e) {}
...
app.get("/api/email-status", (req, res) => {
  try {
    const usingEthereal = mailTransporter && mailTransporter.options && String(mailTransporter.options.host || '').includes('ethereal');
    res.json({ ok: true, usingEthereal, smtpHost: process.env.SMTP_HOST || null });
  } catch (err) {
